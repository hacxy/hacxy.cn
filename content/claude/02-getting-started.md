...
Claude Code 通过 npm 安装，需要 Node.js 18+：

```bash
npm install -g @anthropic-ai/claude-code
```

macOS 用户也可以通过 Homebrew：

```bash
brew install claude-code
```

Windows 用户在 PowerShell 里跑同一条 npm 命令就行，或者用 [WSL](https://learn.microsoft.com/zh-cn/windows/wsl/install)（推荐，体验更接近原生）。
...
第一次运行会提示你登录。按提示用 Claude 账号（或 [Anthropic Console](https://console.anthropic.com) 的 API Key）完成认证。登录成功后，命令行进入交互模式，看到提示符就可以开始用了。
...
这个模式在 CI 脚本里很有用，可以把它接进自动化流程。

## 常见疑问

**Q：它会不会直接删掉我的文件？**

默认情况下，执行高风险操作前会请求你的确认。可以在设置里调整权限策略，具体见[第五章](./05-settings-and-hooks)。
...
代码发送给 Anthropic 的 API 进行处理，适用 [Anthropic 隐私政策](https://anthropic.com/privacy)。企业版（Enterprise）有更严格的数据隔离保证。

## 小结

上手成本很低：一条 npm 安装命令，一次登录，进入项目目录，`claude`。核心使用模式就是在终端里和它对话，告诉它做什么，审查它的修改，确认或拒绝。
