...
JetBrains 插件有几个好用的功能：代码修改会直接在 IDE 的 diff 查看器里展示，当前选中的代码会自动作为上下文传给 Claude，IDE 里的 lint 和语法错误也会自动同步过去。
...
Claude Code 会自己去读文件，然后给你一个概述。这步不是必须的，但有助于你感受它"真的看到了你的代码"——和普通 AI 聊天贴代码片段的感觉完全不同。
...
Claude Code 在修改文件之前，通常会展示 diff 并请求你的确认。看到 diff 后：
...
其实刚才说的"每次都确认"只是默认行为。Claude Code 有好几种权限模式，适合不同的使用场景。按 `Shift+Tab` 可以在模式之间循环切换。

### Default（默认模式）

读文件不需要确认，写文件和执行命令都要你点头。这是最保守的模式，刚上手的时候用这个就对了。

### AcceptEdits（自动接受编辑）

文件修改自动通过，但 Bash 命令还是要确认。适合你已经建立了一定信任，不想每个文件改动都按一次 Enter 的时候。反正改了什么 git diff 一看就知道。

### Plan（规划模式）

Claude 只读代码、做分析、给出方案，但不会动你的文件。适合你想先看看它打算怎么做，确认思路对了再放手让它干活。你可以用 `Shift+Tab` 切到 Plan 模式，也可以在单条消息前加 `/plan` 临时进入规划模式。

说实话，对于复杂任务，先用 Plan 模式让它出方案是个好习惯。
...
Claude Code 的会话会自动保存到磁盘（在 `~/.claude/projects/` 下），所以你可以随时恢复之前的对话。
...
在会话里输入 `/rename my-feature` 就能起个名字。起过名的会话在列表里更容易找到。也可以启动时就指定名称：
...
这些参数在 CI 脚本里很有用，防止 Claude 无限循环烧钱。
...
代码发送给 Anthropic 的 API 进行处理，适用 [Anthropic 隐私政策](https://anthropic.com/privacy)。Enterprise 版有更严格的数据隔离保证。
...
是的。Claude Code 本身只是个客户端，所有推理都在 Anthropic 的服务器上完成，断网没法用。

**Q：能同时开多个 Claude Code 吗？**

可以。你可以在不同的终端窗口、不同的项目目录下同时跑多个 Claude Code 实例，它们互不干扰。如果是同一个项目想并行探索不同方向，用 git worktree 配合效果更好。
