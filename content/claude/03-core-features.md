...
MCP（Model Context Protocol）是一个开放标准，允许 Claude Code 连接外部工具和数据源。通过 MCP，你可以让 Claude Code 直接访问数据库、查询 GitHub Issues、调用内部 API，甚至操作浏览器。
...
Hooks 配置在 `.claude/settings.json` 里，第五章有详细介绍。
...
如果你有一类任务经常重复，比如每次都要告诉 Claude Code "按照 Conventional Commits 规范提交代码"，可以把这个流程打包成 Skill。
...
团队可以把 Skills 提交到 git，所有人共享同一套工作流规范。具体的创建和使用方式参考博客里的《AI Skills》那篇文章。
