...
**Hermes Agent**：Telegram, Discord, Slack, WhatsApp, Signal, Email, SMS, Matrix, Mattermost, Home Assistant, 钉钉(DingTalk), 飞书(Feishu), 企业微信(WeCom), iMessage(BlueBubbles), 微信(Weixin), API Server, Webhooks——通过单一 Gateway 进程统一管理，共 17 个平台。
...
在消息平台覆盖面上，OpenClaw 更广（24+ vs 17）。不过 Hermes 原生支持微信、钉钉、飞书、企业微信，对国内用户来说已经够用。OpenClaw 的优势在于支持 QQ、Teams、LINE 等额外平台。
...
| 消息平台 | 17 个（Telegram, Discord, Slack, WhatsApp, Signal, Email, SMS, Matrix, Mattermost, Home Assistant, 钉钉, 飞书, 企业微信, iMessage, 微信, API Server, Webhooks） | 24+ 个 |
...
2. **消息平台覆盖仍少于 OpenClaw**：虽然已有 17 个平台（包括钉钉、飞书、企业微信、微信），但和 OpenClaw 的 24+ 相比仍有差距，缺少 QQ、IRC、Teams、LINE 等平台的原生支持。
