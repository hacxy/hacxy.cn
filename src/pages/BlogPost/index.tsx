...
function findIsolatedScope(
  items: SidebarItemData[],
  currentPath: string,
): SidebarItemData[] | null {
  for (const item of items) {
...
export default function BlogPost() {
  ...
  const mobileFab = (
    <>
      ...
      {hasSidebar && (
        <div
          className={classNames(styles.mobileDrawer, styles.mobileDrawerLeft, {
            [styles.mobileDrawerOpen]: openDrawer === 'sidebar',
          })}
        >
      ...
      {/* 目录抽屉（右侧） */}
      <div
        className={classNames(styles.mobileDrawer, styles.mobileDrawerRight, {
          [styles.mobileDrawerOpen]: openDrawer === 'toc',
        })}
      >
        <div className={styles.mobileDrawerHeader}>
      ...
      <div className={styles.mobileFab}>
        {hasSidebar && (
          <button
            className={classNames(styles.mobileFabBtn, {
              [styles.mobileFabBtnActive]: openDrawer === 'sidebar',
            })}
            onClick={() =>
              setDrawerState((v) => ({ slug, open: v.open === 'sidebar' ? null : 'sidebar' }))
            }
            aria-label="章节导航"
          >
        ...
        <button
          className={classNames(styles.mobileFabBtn, {
            [styles.mobileFabBtnActive]: openDrawer === 'toc',
          })}
    ...
    return (
      <PageTransition>
        <div className={common.pageWithSidebar}>
          <div className={common.sidebarContent}>{renderContent()}</div>
      ...
      <>
        <article>
          ...
          >
            {post!.series && (
              <Link
                to={`/posts?series=${encodeURIComponent(post!.series)}`}
                className={common.seriesTag}
              >
                {post!.series}
              </Link>
            )}
          ...
          <nav className={styles.postNav}>
            ...
            <div className={`${styles.postNavItem} ${styles.postNavItemRight}`}>
              {next && (
                <Link
                  to={`/${next.slug}`}
                  className={`${styles.postNavLink} ${styles.postNavLinkRight}`}
                >
