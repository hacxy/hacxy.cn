...
/**
 * hero 终端：真实 AI 编码 agent 会话演出（pi 风格）。
 * 结构：会话 transcript → 输入区 → 状态栏；恒黑底白字（与页面主题色解耦）、
 * 四角括号装饰（不进可访问性树）。
 * 每轮演出（数据驱动）：输入框自动逐字符打字 → 发送（问题进入 transcript、
 * 输入框清空）→ Thinking… → Done. → 回答；播放一遍后停驻最终态（不循环）。
 * 演出编排为行级纯 CSS 动画（--delay 按数据确定性计算，SSR 与水合一致）；
 * 预渲染 HTML 含演出全文，爬虫不执行 JS 可读，无 hydration mismatch。
 */

/** 打字机参数：每字符耗时（总时长由文本长度驱动） */
const TYPE_SPEED = 0.13
/** 打字最短时长（输入框打字与行内揭示共用下限） */
const MIN_TYPE = 1.5
...
/** 输入框发送清空时长（与 CSS input-hide 0.2s 保持一致） */
const INPUT_HIDE = 0.2

/** 终端输出行：状态行（Thinking…/Done.）或回答行（文本或任意节点） */
...
/** 由轮次数据计算整场演出时刻表（纯函数）：轮次严格串行，
 *  各轮时刻按节奏参数（INPUT_IN / SEND_PAUSE / THINK_* / ANSWER_*）错开。 */
...
/** 打字机文本（纯 CSS）：完整文本始终在 DOM（基础态不裁切），
 *  animation 期间用 steps(字符数) 逐字符揭示；reduced-motion 下 animation
 *  禁用 → 直接显示完整文本。--type-chars / --type-duration / --delay 由数据驱动。 */
  ...
  return (
    <div className="hero-terminal" role="group" aria-label="AI 会话演出">
      ...
      {/* 输入区：每轮一个 .terminal-typed 绝对定位叠放（文本 clip 打字揭示 → 发送时
          opacity 清空）；输入光标常亮不闪烁、随打字右移；停驻光标末轮发送后出现 */}
      ...
      {/* 状态栏：● live · 文章数 · 标签数 · v<版本>；role=status 内容可被读屏播报 */}
